...
| Path | Purpose |
|------|---------|
| `Projects/` | Primary authored deliverables (active) |
| `reports/` | Optional authored deliverables path |
| `references/` | Registry/plans/shared references |
| `tmp/by-week/` | Weekly plan artifacts |
| `.github/agents/` | Agent definition files |
| `.github/agents/references/` | Shared reference data |
| `.claude/` | Claude workflow templates/checklists |
| `scripts/` | Validation and bridge scripts |
...
- All primary deliverables are authored in `Projects/` (or `reports/` where present) as `Markdown research reports, BibTeX bibliography and Executive summary`
- Compiled output may be generated under project-specific output paths and is **never edited directly**
- Figures are generated from source files in project figure paths — source files are authoritative
...
### Orchestrator
- `@orchestrator` — coordinates all agents; entry point for all user requests after bootstrap (Team Builder may run first only to generate/regenerate the team)
