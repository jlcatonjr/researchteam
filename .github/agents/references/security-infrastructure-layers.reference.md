# Infrastructure Security Layers — ResearchTeam
...
- **Scope gate.** This reference applies only to projects that **build, deploy, run, or operate a
  program, server, or service**. If ResearchTeam ships no deployed software (e.g. a pure
...
- **Honest posture language:** record "engages as tested on ResearchTeam's target," never
...
<!-- Project-owned: record ResearchTeam's chosen layers, tools, deviations, and rationale below.
