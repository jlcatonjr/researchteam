...
This reference defines safe GitHub interaction and merge procedures for ResearchTeam.
...
5. **Cross-repo guard:** if a fix requires pushing to a repository other than `Projects/`, that re-push is a cross-repository write — route it through `@repo-liaison` + `@security` (orchestrator Rule 11) before pushing.
