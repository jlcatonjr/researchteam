...
**Enforcement check:**
```
find Projects/ -name "*.backup" -o -name "*.bak" -o -name "*_old.*" \
...
### CH-03 — No Ad-Hoc Scripts in `Projects/`

Source directories contain only production code. Investigative, debug, fix, and benchmark scripts must not reside in `Projects/`.
...
```
grep -rn "except:\|except Exception" Projects/
```
...
grep -rn "\-\-allow-all-tools" Projects/
```
